...
public void doPost(HttpServletRequest request,HttpServletRespinse response)throws ServletException,IOException{
	Connection con;
	Statement sql;
	...
	try {
		...
		if(boo) {
			...
			if(m=true) {
				...
				RequestDispatcher dispather=request.getRequestDispatcher("login.jsp");    //转发
			...
			String backNews=""+exp;
			fail(request,respone,logname,backNews);
		}
	}
public void doGet(HttpServletRequest request,HttpServletResponse response,String logname,String password) {                   //登录异常处理
	...
	try {  loginBean=(Login)session.getAttribute("loginBean");
	        if(loginBean==null) {
	        	...
	        	loginBean=(Login)session.getAttribute("longinBean");
	        }
